                                       ...
                                       uint32_t numTags, sl::CommandBuffer* cmdBuffer)
{
    ...
    // Preserve the previous pass-through behavior when no resource edits/cache are needed.
    if (State::Instance().activeFgInput != FGInput::NvngxFG &&
        State::Instance().activeFgInput != FGInput::DLSSG && !IsGazeContractCaptureEnabled()) return forward();
