...
void PollInputFallbackLocked()
{
    ...
    {
        // A closed menu only consumes its shortcuts. GetAsyncKeyState enters
        // Win32 for each key, so polling the whole keyboard here can cost
        // milliseconds on every render frame while holding the input mutex.
        // Keep polling held keys until release to avoid stale state when the
        // menu closes or a shortcut is rebound.
        if (!_state.MenuVisible && _state.HiddenMenuPollingConfigured && !_state.HiddenMenuPollingKeys[vk] &&
            !_state.Keys[vk].Down)
            continue;
...
void ResetStateAfterShutdown()
{
    ...
    _state.PolledMouseUsedThisFrame = false;
    _state.PolledKeyboardUsedThisFrame = false;
    _state.HiddenMenuPollingKeys.fill(false);
    _state.HiddenMenuPollingConfigured = false;
...
{
    std::unique_lock lock(_state.Mutex);
    _state.HiddenMenuPollingConfigured = keys != nullptr;
    _state.HiddenMenuPollingKeys.fill(false);
    if (keys == nullptr)
        return;

    for (std::size_t i = 0; i < count; ++i)
        if (keys[i] > 0 && keys[i] < static_cast<int>(_state.HiddenMenuPollingKeys.size()))
            _state.HiddenMenuPollingKeys[keys[i]] = true;

    // Generic Shift/Ctrl/Alt states are synthesized from their left/right
    // keys. Preserve those states and modifier shortcuts in the reduced set.
    for (const int vk : { VK_LSHIFT, VK_RSHIFT, VK_LCONTROL, VK_RCONTROL, VK_LMENU, VK_RMENU })
        _state.HiddenMenuPollingKeys[vk] = true;
}
